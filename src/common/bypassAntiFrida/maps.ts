  ...
  Interceptor.replace(openPtr, new NativeCallback(function (pathname, flag) {
    ...
    if (ch && ch.indexOf('/proc/') >= 0 && ch.indexOf('maps') >= 0) {
      ...
      while (read(FD, MapsBuffer, 512) !== 0) {
        ...
        if (MBuffer) {
          MBuffer = MBuffer.replace('/data/local/tmp/re.frida.server/frida-agent-64.so', 'FakingMaps');
          MBuffer = MBuffer.replace('re.frida.server', 'FakingMaps');
          MBuffer = MBuffer.replace('frida-agent-64.so', 'FakingMaps');
          MBuffer = MBuffer.replace('frida-agent-32.so', 'FakingMaps');
          MBuffer = MBuffer.replace('frida', 'FakingMaps');
          MBuffer = MBuffer.replace('/data/local/tmp', '/data');
